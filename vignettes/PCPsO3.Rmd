...
gp
```
Fig 1: A parallel coordinate plot of the four variables from the Election2005 dataset with the outliers highlighted that are identified for the whole dataset by at least one of the five methods.  Nine outliers have a small area (Flaeche) and a high population density (BDichte), while one is the opposite.
...
gc
```
Fig 2: A parallel coordinate plot of the outlier scores reported by the different methods.  Cases identified as outliers for the full dataset are highlighted.  `PCS` and `BAC` give similar results.
